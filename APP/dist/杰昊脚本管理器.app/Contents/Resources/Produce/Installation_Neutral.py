...
功能：
- 读取 Check/ 文件夹下的产品参数
- 从 Produce/Installation_Content/{受众}/ 文件夹读取内容池
- 使用 Python 代码层进行分层随机抽卡（每次抽取 {CARD_DRAW_COUNT} 个模块）
- 为每种受众类型生成对应文章
- 输出到 Website/{产品}/articles/ 目录
- 备份到 文章/{产品}/Installation/{受众}/article-{N}/

与 Installation_JIEHAO.py 的区别：
- 仅 User Prompt 内容不同（删除暗广，保持中立客观）
- System Prompt 保持不变
"""
...
INSTALLATION_CONTENT_DIR_STR = CONTENT_POOL_DIR["Installation"]
BACKUP_SUBFOLDER = BACKUP_INSTALL_NEUTRAL

# ================= 受众配置 =================
AUDIENCE_CONFIG = {
    "Installation_Technician": {
        "name": "安装技师 (Installation Technician)",
    ...
    "Site_Supervisor": {
        "name": "现场主管 (Site Supervisor)",
        "perspective": "关注交叉作业协调与进度管控",
        "focus_1": "安装顺序与工序交接节点",
        "focus_2": "洁净区安装期间污染控制",
        "focus_3": "安装完成面的交接验收标准"
    },
    "Commissioning_Engineer": {
        "name": "调试工程师 (Commissioning Engineer)",
        "perspective": "关注系统联动调试与性能验证",
        "focus_1": "HVAC互锁与BMS通信联调",
        "focus_2": "压差梯度现场验证",
        "focus_3": "IQ/OQ性能测试执行要点"
    },
    "Facilities_Manager": {
        "name": "设施经理 (Facilities Manager)",
        "perspective": "关注到场条件、交接文件与运维启动",
        "focus_1": "设备到场接收条件确认",
        "focus_2": "土建基础与安装空间核查",
        "focus_3": "运维培训与备件交接"
    },
    "Subcontractor": {
        "name": "分包商 (电气/HVAC分包商)",
        "perspective": "关注接口规格与配合时序",
        "focus_1": "电气接口与BMS协议配置",
        "focus_2": "风管法兰连接与密封标准",
        "focus_3": "互锁控制逻辑技术交接"
    }
}
...
# ================= 从 Prompt 模块导入提示词 =================
from Prompt.Installation_Neutral_User_Prompt import USER_PROMPT_TEMPLATE
from Prompt.Installation_Neutral_System_Prompt import get_installation_article_system_prompt
...
def load_content_pool(audience_type):
    ...
    pool_dir = os.path.join(INSTALLATION_CONTENT_DIR_STR, audience_type)
...
def draw_cards(content_pool, k=5):
    """从内容池中随机抽取 k 个模块（不放回抽取，池小于 k 时抽取全部）"""
    if not content_pool:
        return []
    draw_count = min(len(content_pool), k)
    selected = random.sample(content_pool, k=draw_count)
    return selected
...
def infer_thematic_thread(selected_contents, audience_type):
    """从模块中推断安装调试主题线索"""
    if audience_type == "Installation_Technician":
        return "executing the correct mechanical and pneumatic installation sequence to achieve airtight integrity and fail-safe operation on first attempt"
    elif audience_type == "Site_Supervisor":
        return "coordinating cross-trade installation sequence and managing site handover checkpoints to prevent costly rework and contamination events"
    elif audience_type == "Commissioning_Engineer":
        return "validating system integration and performance parameters through systematic commissioning to satisfy IQ/OQ validation requirements"
    elif audience_type == "Facilities_Manager":
        return "verifying site readiness conditions, installation acceptance criteria, and handover documentation completeness before operational turnover"
    elif audience_type == "Subcontractor":
        return "interpreting interface specifications and integration requirements to ensure seamless electrical and HVAC connection with containment equipment"
    return "executing a complete and compliant installation and commissioning sequence for biosafety containment equipment"
...
def generate_article_for_audience(
    ...
    try:
        ...
        format_args = {
            ...
            'chapter_structure_additional': '\n'.join([f'## {i+2}. [Installation Step {i+1} — Title Written by You]' for i in range(1, CARD_DRAW_COUNT)]),
        }
        ...
        # 构建 System Prompt
        system_prompt = get_installation_article_system_prompt(current_time)

        print(f"  ⏳  正在请求 AI 生成文章 (受众: {audience_type})...")

        # ===== 测试模式：仅输出 prompt，不调用 API =====
        _TEST_MODE = os.environ.get("INSTALLATION_TEST_MODE", "").strip().lower() in ("1", "true", "yes")
        if _TEST_MODE:
            if product_name != "生物安全充气气密门" or audience_type != "Installation_Technician":
                # 测试模式仅保留"生物安全充气气密门 + Installation_Technician"
                return log_info
        ...
        if not title:
            title = f"{english_product_name}: Installation and Commissioning Guide"
        ...
        description = f"Complete installation and commissioning guide for {product_name}. Step-by-step setup, site preparation, and validation requirements for cleanroom biosafety equipment."
...
def process_single_product(product_name, product_info):
    ...
    for audience_type, audience_config in AUDIENCE_CONFIG.items():
        _TEST_MODE = os.environ.get("INSTALLATION_TEST_MODE", "").strip().lower() in ("1", "true", "yes")
        if _TEST_MODE and (product_name != "生物安全充气气密门" or audience_type != "Installation_Technician"):
            continue
        print(f"\n  📋 【处理受众】: {audience_type}")
...
def main():
    print("=" * 60)
    print("  Installation 文章生成器 - Content Pool Card-Drawing Generator")
    print("  (中立版 / Neutral Version)")
    print("=" * 60)
    ...
    if AUTO_GIT_PUSH and success_count > 0:
        print("\n" + "=" * 60)
        commit_msg = f"Auto generate: {success_count} installation article(s) with card-drawing (neutral)"
