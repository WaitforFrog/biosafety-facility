...
-- 脚本列表：脚本名 -> 描述（Produce 目录下的脚本使用相对路径）
set scriptList to {{"Produce/Trust_EN_html.py", "翻译并生成英文HTML（中立科普）"}, {"Produce/Compare_EN_html.py", "市场分析类文章生成器"}, {"Translate.py", "翻译脚本"}, {"Produce/Tool/build_preview_site.py", "构建预览站点"}, {"Produce/Tool/build_category_indexes.py", "构建分类索引"}, {"Produce/Tool/build_articles_index.py", "构建文章索引"}, {"delete_chinese_articles.py", "删除中文文章"}}
