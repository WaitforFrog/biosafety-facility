...
# 内容池基础目录
CONTENT_POOL_DIR = "/Users/guot/Desktop/杰昊/AI推广/域名推广/Code/Produce/Question_Content"
# 文章备份子文件夹
BACKUP_SUBFOLDER = "Question_Neutral"
# 每次抽卡数量
CARD_DRAW_COUNT = 4
# 每受众生成文章数量
...
# ================= 从 Prompt 模块导入提示词 =================
from Prompt.Question_Neutral_User_Prompt import USER_PROMPT_TEMPLATE
from Prompt.Question_Neutral_System_Prompt import get_question_article_system_prompt
    ...
    try:
        ...
        if _TEST_MODE:
            if product_name != "不锈钢密闭房" or audience_type != "CEO":
                # 测试模式仅保留"不锈钢密闭房 + CEO"
                return log_info
            test_path = "/Users/guot/Desktop/杰昊/AI推广/域名推广/Code/Test_Question_Neutral.md"
            with open(test_path, 'w', encoding='utf-8') as f:
                ...
                f.write(user_prompt)
                f.write("\n```\n")
            print(f"  ✅ Test_Question_Neutral.md 已生成: {test_path}")
...
def main():
    print("=" * 60)
    print("  Question Neutral 文章生成器 - Content Pool Card-Drawing Generator")
    print("=" * 60)
    ...
    if AUTO_GIT_PUSH and success_count > 0:
        print("\n" + "=" * 60)
        commit_msg = f"Auto generate: {success_count} question neutral article(s) with card-drawing"
