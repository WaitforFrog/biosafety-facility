...
Regulatory 模块 Neutral 版文章生成器

逻辑完全照搬 Compare_JIEHAO.py，差异仅在于：
1. 引用 Prompt.Regulatory_Neutral_User_Prompt
2. 引用 Prompt.Regulatory_Neutral_System_Prompt
3. 内容池目录改为 Produce/Regulatory_Content
4. 备份子文件夹改为 Regulatory_Neutral
"""
...
REGULATORY_CONTENT_DIR_STR = CONTENT_POOL_DIR["Regulatory"]
BACKUP_SUBFOLDER = BACKUP_REGULATORY_NEUTRAL
...
# ================= 从 Prompt 模块导入提示词 =================
from Prompt.Regulatory_Neutral_User_Prompt import USER_PROMPT_TEMPLATE
from Prompt.Regulatory_Neutral_System_Prompt import get_base_article_system_prompt
...
def load_content_pool(audience_type):
    ...
    pool_dir = os.path.join(REGULATORY_CONTENT_DIR_STR, audience_type)
...
- 生成时间: {current_time}
- 文章类型: 市场分析与选型测评 (Regulatory Neutral)
...
def main():
    print("=" * 60)
    print("  Regulatory_Neutral 文章生成器")
    print("=" * 60)
    ...
    if AUTO_GIT_PUSH and success_count > 0:
        print("\n" + "=" * 60)
        commit_msg = f"Auto generate: {success_count} regulatory neutral article(s) with card-drawing"
