...
sys.path.insert(0, str(PRODUCE_DIR / "Tool"))
...
def call_api(system_prompt, user_prompt, temperature=TRUST_TEMPERATURE, max_tokens=TRUST_MAX_TOKENS):
    """调用 OpenAI API"""
    try:
...
Request timestamp: {current_time} (For reference only, do not mention in article.)"""
        ...
        result_text = call_api(system_prompt, user_prompt, TRUST_TEMPERATURE, max_tokens=TRUST_MAX_TOKENS)
...
print(f"\n✅ 共加载 {len(products)} 个产品，开始并行生成中立科普文章（并发数: {TRUST_MAX_CONCURRENT}）...\n")

# 并行处理
results = []
with ThreadPoolExecutor(max_workers=TRUST_MAX_CONCURRENT) as executor:
    # 提交所有任务
    future_to_product = {
